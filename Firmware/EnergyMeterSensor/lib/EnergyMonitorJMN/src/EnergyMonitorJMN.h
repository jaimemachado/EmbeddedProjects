...
#ifndef _ENERGYMONITOR_h
#define _ENERGYMONITOR_h
...
public:    
    ...
    void addWattRead(double readWatt)
    {
        ...
        if (numberOfAdds == NUMBER_READS_TO_UPDATE_KWH)
        {
            ...
            tempKWH = (double)((accumulateKW / numberOfAdds)*milis) + tempKWH;
        ...
        if (tempKWH > (3600000000 * POWER_METER_PRECISION))
        {
            ...
            {
                tempKWH = tempKWH - (double)(3600000000 * POWER_METER_PRECISION);
                kwh = (double)POWER_METER_PRECISION + kwh;
                if(isDay_)
                {
